...
function adjustHeight() {
	var currentHeight = parseInt(document
			.querySelector(".original-in-monaco-diff-editor").style.height);
	if (parseInt(bubbleBottom) > parseInt(currentHeight)) {
		currentHeight = bubbleBottom;
		document.querySelector(".original-in-monaco-diff-editor").style.height = currentHeight
				+ "px";
		...
		document.querySelector(".modified-in-monaco-diff-editor").style.height = currentHeight
				+ "px";
		document
				.querySelector(".original-in-monaco-diff-editor .monaco-scrollable-element.editor-scrollable.vs").style.height = currentHeight
				+ "px";
		document
				.querySelector(".modified-in-monaco-diff-editor .monaco-scrollable-element.editor-scrollable.vs").style.height = currentHeight
				+ "px";
		document
				.querySelector(".original-in-monaco-diff-editor .lines-content.monaco-editor-background").style.height = currentHeight
				+ "px";
		document
				.querySelector(".modified-in-monaco-diff-editor .lines-content.monaco-editor-background").style.height = currentHeight
				+ "px";
	}
}
...
function generateOriginalEditor(wholeHeight) {
	...
	document.querySelector(".original-in-monaco-diff-editor").appendChild(
			overlaysDiv);
	document.querySelector(".original-in-monaco-diff-editor").style.height = wholeHeight
			+ "px";
	// 行中的对齐线；删除、增加颜色效果
	...
	backgroundDiv.style = "overflow: hidden; height: "
			+ wholeHeight + "px; will-change: transform; top: 0px;";
	...
	viewOverlaysDiv.style = "height: 0px; width: 100%;";
	...
	linesDiv1.style = "font-family: Consolas, monospace; font-weight: normal; font-size: 14px; line-height: 19px; letter-spacing: 0px; width: 538px; height: 4523px;";
	...
	generateEditor(originalLines, originalLinesCoordinate, overlaysDiv,
			viewOverlaysDiv, linesDiv1, viewZonesDiv1, 1);
}

function generateEditorTest(contentLines, linesCoordinate, overlaysDiv,
		viewOverlaysDiv, linesDiv, viewZonesDiv, sign) {
	var diagonalLine = 0;
	...
	newDiv2.innerHTML = "";
	var content = "**";
	newDiv3.innerHTML = "<span style='float:left'><span class='mtk1'>"
			+ content + "</span></span>";
...
function generateModifiedEditor(wholeHeight) {
	...
	document.querySelector(".modified-in-monaco-diff-editor").appendChild(
			overlaysDiv2);
	// 行中的对齐线；删除、增加颜色效果
	...
	document.querySelector(".modified-in-monaco-diff-editor").appendChild(
			scrollableDiv2);
	document.querySelector(".modified-in-monaco-diff-editor").style.height = wholeHeight
			+ "px";
	var backgroundDiv2 = document.createElement("div");
	backgroundDiv2.className = "lines-content monaco-editor-background";
	backgroundDiv2.style = "overflow: hidden; height: "
			+ wholeHeight + "px; will-change: transform; top: 0px;";
	...
	viewOverlaysDiv2.style = " height: 0px;width: 100%;";
	...
	linesDiv2.style = " font-family: Consolas, monospace; font-weight: normal; font-size: 14px; line-height: 19px; letter-spacing: 0px; width: 538px; height: 4523px;";
	...
	generateEditor(modifiedLines, modifiedLinesCoordinate, overlaysDiv2,
			viewOverlaysDiv2, linesDiv2, viewZonesDiv2, 2);
}

function generateEditor(contentLines, linesCoordinate, overlaysDiv,
		viewOverlaysDiv, linesDiv, viewZonesDiv, sign) {
	// 阴影行数
	var diagonalLine = 0;
	...
	for (var l = 0; l < contentLines.length; l++) {
		...
		if (contentLines[l].number == undefined) {
			...
			if (diagonalLine != 0) {
				...
				diagonalDiv.style = " width: 100%; display: block; top: "
						+ (19 * (l - diagonalLine))
						+ "px; height: "
						+ (19 * diagonalLine) + "px;";
			...
			var inner;
			if (contentLines[l].mark != undefined
					&& (contentLines[l].mark == "insert"
							|| contentLines[l].mark == "change"
							|| contentLines[l].mark == "move" || contentLines[l].mark == "delete")) {
				var mark = contentLines[l].mark;
				if ((contentLines[l].mark == "move" || contentLines[l].mark == "change")
						&& sign == 2)
					mark = "";
				newDiv1.innerHTML = "<div class='cmdr line-"
						+ mark
						+ "' style='height: 19px'></div><div class='cldr "
						+ contentLines[l].mark
						+ "-sign' style='left: 38px; width: 15px; height: 19px'></div><div class='line-numbers'>"
						+ contentLines[l].number + "</div>";
				inner = "<div class='cdr line-"
						+ contentLines[l].mark
...
// inner += "<li><div style='background-color: "+getColorByType(types[i])+";'
// class='legend_icon'></div><span>"+types[i]+"</span></li>";
// inner +="</ul>";
//	
