import { JsPlumbDefaults, UICoreOptions } from "./defaults";
...
import { ConnectionMovedParams, ConnectionEstablishedParams } from "./callbacks";
...
import { Connector, PaintStyle } from "../common/index";
import { OptimisticEventGenerator } from "../util/event-generator";
...
export interface jsPlumbElement<E> {
    ...
    _jsPlumbProxies: Array<[Connection<E>, number]>;
...
export declare abstract class JsPlumbInstance<T extends {
    ...
    readonly connections: Array<Connection<T["E"]>>;
    endpointsByElement: Record<string, Array<Endpoint<T["E"]>>>;
    ...
    constructor(toolkit: JsPlumbToolkit, options: UICoreOptions<T["E"]>, defaults?: JsPlumbDefaults<T["E"]>);
    /**
     * @internal
     */
    ...
    getConnections(options?: SelectOptions<T["E"]>, flat?: boolean): Record<string, Connection<T["E"]>> | Array<Connection<T["E"]>>;
     ...
     * @param el
     */
    setSource(connection: Connection<T["E"]>, el: T["E"] | Endpoint<T["E"]>): void;
    /**
     * Change the target of the given connection to be the given endpoint or element.
     * @param connection
     * @param el
     */
    setTarget(connection: Connection<T["E"]>, el: T["E"] | Endpoint<T["E"]>): void;
    /**
     * Sets the type of a connection and then repaints it.
     * @param connection
     * @param type
     */
    setConnectionType(connection: Connection<T["E"]>, type: string, params?: any): void;
    /**
     ...
     */
    deleteConnection(connection: Connection<T["E"]>, params?: DeleteConnectionOptions): boolean;
     ...
     * added by jsPlumb to mark it as managed.
     ...
     * @internal
     */
    _internal_newEndpoint(params: InternalEndpointOptions<T["E"]>): Endpoint<T["E"]>;
    /**
     ...
     * @internal
     */
    _maybePruneEndpoint(endpoint: Endpoint<T["E"]>): boolean;
    /**
     * Delete the given endpoint.
     * @param object - Either an Endpoint, or the UUID of an Endpoint.
     */
    deleteEndpoint(object: string | Endpoint<T["E"]>): JsPlumbInstance;
    /**
     ...
     */
    addEndpoint(el: T["E"], params?: EndpointOptions<T["E"]>, referenceParams?: EndpointOptions<T["E"]>): Endpoint<T["E"]>;
    /**
     ...
     * @param referenceParams
     */
    addEndpoints(el: T["E"], endpoints: Array<EndpointOptions<T["E"]>>, referenceParams?: EndpointOptions<T["E"]>): Array<Endpoint<T["E"]>>;
    /**
     ...
     * @param el
     */
    getEndpoints(el: T["E"]): Array<Endpoint<T["E"]>>;
    /**
     * Retrieve an endpoint by its UUID.
     * @param uuid
     */
    getEndpoint(uuid: string): Endpoint<T["E"]>;
    /**
     ...
     */
    connect(params: ConnectParams<T["E"]>, referenceParams?: ConnectParams<T["E"]>): Connection<T["E"]>;
    /**
     * @param params
     * @param referenceParams
     * @internal
     ...
     * @internal
     */
    _newConnection(params: ConnectionOptions<T["E"]>): Connection<T["E"]>;
    /**
     ...
     * @internal
     */
    _finaliseConnection(jpc: Connection<T["E"]>, params?: any, originalEvent?: Event): void;
    /**
     ...
     */
    proxyConnection(connection: Connection<T["E"]>, index: number, proxyEl: T["E"], endpointGenerator: (c: Connection<T["E"]>, idx: number) => EndpointSpec, anchorGenerator: (c: Connection<T["E"]>, idx: number) => AnchorSpec): void;
    /**
     * @internal
     * @param connection
     * @param index
     */
    unproxyConnection(connection: Connection<T["E"]>, index: number): void;
    /**
     * @internal
     * @param originalId
     * @param newId
     ...
     * @param index
     */
    sourceOrTargetChanged(originalId: string, newId: string, connection: Connection<T["E"]>, newElement: T["E"], index: number): void;
     ...
     * @public
     */
    addGroup(group: Group, params: AddGroupOptions<T["E"]>): UIGroup<T["E"]>;
    /**
     ...
     * @private
     */
    _paintEndpoint(endpoint: Endpoint<T["E"]>, params: {
        timestamp?: string;
     ...
     * @param params
     */
    _paintConnection(connection: Connection<T["E"]>, params?: {
        timestamp?: string;
    }): void;
    /**
     * @internal
     * @param endpoint
     */
    _refreshEndpoint(endpoint: Endpoint<T["E"]>): void;
    /**
     ...
     * @public
     */
    addOverlay<E>(component: Connection<E>, overlay: OverlaySpec, doNotRevalidate?: boolean): Overlay;
    /**
     * Remove the overlay with the given id from the given connection.
     ...
     * @public
     */
    setOutlineColor(conn: Connection<T["E"]>, color: string): void;
    /**
     ...
     * @public
     */
    setOutlineWidth(conn: Connection<T["E"]>, width: number): void;
    /**
     ...
     * @public
     */
    setColor(conn: Connection<T["E"]>, color: string): void;
    /**
     ...
     * @public
     */
    setLineWidth(conn: Connection<T["E"]>, width: number): void;
    /**
     ...
     * @public
     */
    setLineStyle(conn: Connection<T["E"]>, style: {
        lineWidth?: number;
        outlineWidth?: number;
        color?: string;
        outlineColor?: string;
    ...
    abstract beforeDrop(params: {
        connection: Connection<T["E"]>;
        dropEndpoint: Endpoint<T["E"]>;
    }): any;
    abstract beforeStartDetach(params: {
        endpoint: Endpoint<T["E"]>;
        connection: Connection<T["E"]>;
    }): Record<string, any> | boolean;
    abstract beforeDetach(connection: Connection<T["E"]>, isDiscard?: boolean): boolean;
    abstract beforeDrag(params: {
        endpoint: Endpoint<T["E"]>;
    }): Record<string, any> | boolean;
    abstract on(el: Document | Element | T["E"] | ArrayLike<T["E"]>, event: string, callbackOrSelector: Function | string, callback?: Function): void;
    abstract off(el: Document | Element | T["E"] | ArrayLike<T["E"]>, event: string, callback: Function): void;
    abstract trigger(el: Document | Element | T["E"], event: string, originalEvent?: Event, payload?: any, detail?: number): void;
    getModelObjectFromEndpoint(ep: Endpoint<T["E"]>): import("../../core/model/graph").Node | import("../../core/model/graph").Port;
    /**
     * @internal
     * @param connector
     */
    getPathData(connector: Connector): any;
    /**
     * @internal
     * @param o
     * @param params
    ...
    abstract drawOverlay(overlay: Overlay, component: Connector, paintStyle: PaintStyle, absolutePosition?: PointXY): any;
    abstract reattachOverlay(o: Overlay, c: Connection<T["E"]>): void;
    abstract setOverlayHover(o: Overlay, hover: boolean): void;
    abstract setHover(component: ComponentBase, hover: boolean): void;
     ...
     * @param extents
     */
    abstract paintConnector(connector: Connector, paintStyle: PaintStyle, extents?: Extents): void;
    /**
     * @internal
     * @param connection
     * @param force
     */
    abstract destroyConnector(connection: Connection<T["E"]>, force?: boolean): void;
    /**
     * @internal
     * @param connector
     * @param h
     ...
     */
    abstract setConnectorHover(connector: Connector, h: boolean, sourceEndpoint?: Endpoint<T["E"]>): void;
    /**
     * @internal
     * @param connector
     * @param clazz
    ...
    abstract applyConnectorType(connector: Connector, t: TypeDescriptor): void;
    abstract applyEndpointType(ep: Endpoint<T["E"]>, t: TypeDescriptor): void;
    abstract setEndpointVisible(ep: Endpoint<T["E"]>, v: boolean): void;
    abstract destroyEndpoint(ep: Endpoint<T["E"]>): void;
    abstract renderEndpoint(ep: Endpoint<T["E"]>, paintStyle: PaintStyle): void;
    abstract addEndpointClass(ep: Endpoint<T["E"]>, c: string): void;
    abstract removeEndpointClass(ep: Endpoint<T["E"]>, c: string): void;
    abstract getEndpointClass(ep: Endpoint<T["E"]>): string;
    abstract setEndpointHover(endpoint: Endpoint<T["E"]>, h: boolean, endpointIndex: number, doNotCascade?: boolean): void;
}
